...
import { toCelsius } from '../../../Utils/toCelsiusfromKelvin';
...
const WeatherMain: FC<IPropsType> = ({ params, currentWeather }) => {
  ...
  const pop = currentWeather.weather[0].pop * 100;
  const humidity = currentWeather.weather[0].main.humidity;
  const windSpeed = currentWeather.weather[0].wind.speed;
  const temp = toCelsius(weatherAverage(currentWeather.weather));
